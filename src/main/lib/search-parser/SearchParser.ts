...
export class SearchParser {
  ...
  parse(query: string): SearchQuery {
    ...
    // unnamed list is for text index searching. That may include title, artist, difficulty, and more
    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      // Check if token is simple (DragonForce) or with relation symbol (artist=xi)
