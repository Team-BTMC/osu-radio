    ...
    <>
      ...
      <button class={"hint"} onClick={switchDirections} disabled={props.disabled} title={direction() === "asc" ? "Ascending" : "Descending"}>
        <Show
          ...
          fallback={
            <Show when={direction() === "asc"} fallback={<Fa icon={faArrowDown91} scale={GLOBAL_ICON_SCALE}/>}>
              <Fa icon={faArrowDown19} scale={GLOBAL_ICON_SCALE}/>
            </Show>
          }
        >
          <Show when={direction() === "asc"} fallback={<Fa icon={faArrowDownZA} scale={GLOBAL_ICON_SCALE}/>}>
