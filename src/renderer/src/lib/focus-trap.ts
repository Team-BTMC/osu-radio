...
const createFocusTrap = (props: {
  ...
  const [focusableElements, setFocusableElements] = createSignal<HTMLElement[] | null>(null);
  ...
  const loadFocusTrap = (container: HTMLElement) => {
    setFocusableElements(
      Array.from(container.querySelectorAll(focusableElementSelector)) as HTMLElement[],
    );
  };

  const initialFocus = (container: HTMLElement) => {
    const initialFocusElement =
      access(defaultedProps.initialFocusElement) ?? firstFocusElement() ?? container;
